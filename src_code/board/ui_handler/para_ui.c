...
Item Para_Setting_Status_Item[] =
{
    ...
    {139, 163,  "低", 	WHITE, BLACK, 24, 1},	//4
    {139, 163,  "中", 	WHITE, BLACK, 24, 1},	//5
    {139, 163,  "高", 	WHITE, BLACK, 24, 1},	//6
};
...
void para_item_display(uint8_t enable)
{
	LCD_Ascii_Show_Para ascii_colon_str[] = 
	{
		...
		{100, 163, 125, ":", BLACK, WHITE, 24},/*显示灵敏度冒号*/
		
		{100, 103, 125, ":", BLACK, BLACK, 24},
		{100, 134, 125, ":", BLACK, BLACK, 24},
		{100, 163, 125, ":", BLACK, BLACK, 24},
		
		{139 + 40, 163, 50, para_setting_page_ui.display_buf, BLACK, BLACK, 24},
	};
    if(enable == 1)
    {
        /*显示菜单*/
    ...
    else if(enable == 0)
    {
		...
		/*隐藏灵敏度数值显示*/
		lcd_model.lcd_driver->lcd_show_ascii_str(ascii_colon_str[6]);
    }
}
...
{
    User_Memory_Para.alarm		= para_setting_page_ui.alarm ;
    User_Memory_Para.value		= para_setting_page_ui.value ;
    User_Memory_Para.sensivity	= para_setting_page_ui.sensivity ;

    for(int i = 0 ; i < 3 ; i++)
        User_Memory_Para.alarm_threshold[i] = para_setting_page_ui.sensivity_value[i];
	
	/*保存配置参数*/
	setting_alarm(User_Memory_Para.alarm);
    setting_value(User_Memory_Para.value);
    setting_sensivity(User_Memory_Para.sensivity);
	para_item_display(0);
